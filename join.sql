...
SELECT genres.id, genres.name, SUM(tracks.milliseconds) AS total_track_length
FROM genres
JOIN tracks
  ON (tracks.genre_id = genres.id)
GROUP BY genres.id
...
SELECT genres.id, genres.name, AVG(tracks.milliseconds) AS avg_track_length
FROM genres
JOIN tracks
  ON (tracks.genre_id = genres.id)
GROUP BY genres.id
...
SELECT albums.id, albums.title, SUM(tracks.milliseconds) AS total_track_length
FROM albums
JOIN tracks
  ON (tracks.genre_id = albums.id)
GROUP BY albums.id
ORDER BY total_track_length DESC
LIMIT 5;

-- The top 5 albums by average track length
SELECT albums.id, albums.title, AVG(tracks.milliseconds) AS avg_track_length
FROM albums
JOIN tracks
  ON (tracks.genre_id = albums.id)
GROUP BY albums.id
...
SELECT albums.id, albums.title, SUM(tracks.unit_price) AS total_album_price
FROM albums
JOIN tracks
...
SELECT albums.id, albums.title, SUM(tracks.milliseconds) AS total_play_time
FROM albums
JOIN tracks
  ON (tracks.album_id = albums.id)
GROUP BY albums.id
...
SELECT tracks.id, tracks.name, SUM(invoice_lines.quantity) AS total_sales
FROM tracks
JOIN invoice_lines
